...
class CreateCheckoutComponent extends Component {
    ...
    componentDidMount(){
        ...
        // step 4. in componentDidMount() method, if the id is "create" then we don't do anything else we retrieve dish by id using CheckoutService.getDataById() method.
        if(this.state.id === 'create'){
            return
        }else{
            CheckoutService.getDataById(this.state.id).then( (res) =>{
                let data = res.data;
                this.setState({
    ...
    saveOrUpdateDish = (e) => {
        ...
        /*
            step 5.
            in the saveOrUpdateDish() method, we check if the id is "create" then we call CheckoutService.create() method which
            internally makes a rest API call to store dish data into mysql database. if id is any positive number than we call
            CheckoutService.updateDataById() method which internally makes a rest API call to store updated dish into mysql database.
        */
        if(this.state.id === 'create'){
            CheckoutService.create(dish).then(res =>{
                this.props.history.push('/checkout');
            });
        }else{
            CheckoutService.updateDataById(dish, this.state.id).then( res => {
                this.props.history.push('/checkout');
            });
        }
    }
    ...
    getTitle(){
        if(this.state.id === 'create'){
            return <h3 className="text-center">Add Checkout</h3>
        }else{
            return <h3 className="text-center">Update Checkout</h3>
        }
    }
    render() {
        return (
...
export default CreateCheckoutComponent